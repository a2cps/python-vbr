    ...
    for column_name, column_attrs in defn.get("columns", {}).items():
        ...
        if col["column_type"] == "varchar":
            if column_attrs.get("char_len", None) is not None:
                col["column_type"] = "varchar({0})".format(column_attrs.get("char_len"))
...
if __name__ == "__main__":
    ...
    ALLOW_LIST = [
        ...
        "subject",
        "unit",
        "container_in_shipment",
        "data_event_in_subject",
        "data_event_in_biosample",
        "data_event_in_measurement",
    ...
    parser.add_argument(
        "--dotfile", action="store_true", help="Also generate a dotfile output"
    )
