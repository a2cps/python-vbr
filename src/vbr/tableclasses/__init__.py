...
def class_from_linkage(child_table_name: str, parent_table_name: str) -> record.VBRRecord:
    """Return VBR linkage class by child and parent table names
    """
    for c in _classes():
        try:
            if c.parent() == parent_table_name and c.child() == child_table_name:
                return c
        except Exception:
            pass
    raise errors.LinkageNotSupported(
        'Linking "{0}" => "{1}" is not currently supported by the VBR module'.format(
