...
class RcapSubjectAssociation(object):
    ...
    subject_id = Column(
        Integer, ForeignKey("subject.subject_id"), nullable=True)

class RcapBiosampleAssociation(object):
    """Associate a REDcap table entry to a VBR.biosample"""
    sample_id = Column(
        Integer, ForeignKey("biosample.biosample_id"), nullable=True)

class RcapDataEventAssociation(object):
    """Associate a REDcap table entry to a VBR.data_event"""
    data_event_id = Column(
        Integer, ForeignKey("data_event.data_event_id"), nullable=True)

class RcapTable(RcapSubjectAssociation, RcapBiosampleAssociation, RcapDataEventAssociation, Table):
