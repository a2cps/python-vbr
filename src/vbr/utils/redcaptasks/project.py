...
def get_secret_map(tapis_client: object,
    ...
    if tenant is None:
        tenant = os.environ.get(TAPIS_SECRETS_TENANT,
                                getattr(tapis_client, 'tenant_id', TENANT))

    return tapis_client.sk.readSecret(
        secretType='user',
        secretName=secret_name,
        tenant=tenant,
        user=secret_owner).get('secretMap').__dict__
...
def redcap_project_to_token(project_id: str,
                            tapis_client: object,
                            secret_owner: str = None,
                            secret_name: str = None,
                            tenant: str = None) -> str:
    try:
        token = get_secret_map(tapis_client=tapis_client,
                               secret_owner=secret_owner,
                               secret_name=secret_name,
                               tenant=tenant).get(str(project_id))
        return token
    except KeyError:
        raise ValueError('No token for {0} found in {1}'.format(
